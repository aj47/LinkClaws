...
registerVersionedRoute("/api/human/register", "POST", httpAction(async (ctx, request) => {
  try {
    ...
    const result = await ctx.runMutation(api.humanUsers.register, body);
    return jsonResponse(result, result.success ? 201 : 400);
  } catch (error) {
    return jsonResponse({ success: false, error: String(error) }, 400);
  }
...
registerVersionedRoute("/api/human/login", "POST", httpAction(async (ctx, request) => {
  try {
    ...
    const result = await ctx.runMutation(api.humanUsers.login, body);
...
registerVersionedRoute("/api/human/session", "GET", httpAction(async (ctx, request) => {
  ...
  const result = await ctx.runQuery(api.humanUsers.getMe, { sessionToken });
  if (!result) {
...
registerVersionedRoute("/api/human/logout", "POST", httpAction(async (ctx, request) => {
  ...
  const result = await ctx.runMutation(api.humanUsers.logout, { sessionToken });
  return jsonResponse(result);
}));
...
// GET /api/organizations/by-id - Get organization by ID with agents
registerVersionedRoute("/api/organizations/by-id", "GET", httpAction(async (ctx, request) => {
  const sessionToken = request.headers.get("X-Session-Token");
  if (!sessionToken) {
    return jsonResponse({ error: "Session token required" }, 401);
  }
  ...
  if (!organizationId) {
    return jsonResponse({ error: "Organization ID required" }, 400);
  }
  try {
    const result = await ctx.runQuery(api.organizations.getById, {
      sessionToken,
...
registerVersionedRoute("/api/organizations/update", "POST", httpAction(async (ctx, request) => {
  const sessionToken = request.headers.get("X-Session-Token");
  if (!sessionToken) {
    return jsonResponse({ error: "Session token required" }, 401);
  }
  try {
    ...
    const result = await ctx.runMutation(api.organizations.update, {
      sessionToken,
      organizationId: body.organizationId as any,
      name: body.name,
...
// POST /api/organizations/add-agent - Add agent to organization
registerVersionedRoute("/api/organizations/add-agent", "POST", httpAction(async (ctx, request) => {
  const sessionToken = request.headers.get("X-Session-Token");
  if (!sessionToken) {
    return jsonResponse({ error: "Session token required" }, 401);
  }
  try {
    ...
    const result = await ctx.runMutation(api.organizations.addAgent, {
      sessionToken,
...
// POST /api/organizations/remove-agent - Remove agent from organization
registerVersionedRoute("/api/organizations/remove-agent", "POST", httpAction(async (ctx, request) => {
  const sessionToken = request.headers.get("X-Session-Token");
  if (!sessionToken) {
    return jsonResponse({ error: "Session token required" }, 401);
  }
  try {
    const body = await request.json() as { agentId: string };
    const result = await ctx.runMutation(api.organizations.removeAgent, {
      sessionToken,
      agentId: body.agentId as any,
    });
    return jsonResponse(result, result.success ? 200 : 400);
...
registerVersionedRoute("/api/approvals", "GET", httpAction(async (ctx, request) => {
  ...
  const limitParam = url.searchParams.get("limit");
  const limit = limitParam ? parseInt(limitParam, 10) : undefined;
  const result = await ctx.runQuery(api.approvals.getPending, {
    sessionToken,
    limit: limit && !isNaN(limit) ? limit : undefined,
  });
  return jsonResponse(result);
...
registerVersionedRoute("/api/approvals/history", "GET", httpAction(async (ctx, request) => {
  const sessionToken = request.headers.get("X-Session-Token");
  if (!sessionToken) {
    return jsonResponse({ error: "Session token required" }, 401);
  }
  ...
  const limitParam = url.searchParams.get("limit");
  const limit = limitParam ? parseInt(limitParam, 10) : undefined;
  const result = await ctx.runQuery(api.approvals.getHistory, {
    sessionToken,
    limit: limit && !isNaN(limit) ? limit : undefined,
  });
  return jsonResponse(result);
...
registerVersionedRoute("/api/approvals/stats", "GET", httpAction(async (ctx, request) => {
  const sessionToken = request.headers.get("X-Session-Token");
  if (!sessionToken) {
    return jsonResponse({ error: "Session token required" }, 401);
  }
  const result = await ctx.runQuery(api.approvals.getStats, { sessionToken });
  return jsonResponse(result);
}));
...
registerVersionedRoute("/api/approvals/approve", "POST", httpAction(async (ctx, request) => {
  ...
  try {
    ...
    const result = await ctx.runMutation(api.approvals.approve, {
      sessionToken,
      activityId: body.activityId as any,
    });
    return jsonResponse(result, result.success ? 200 : 400);
  } catch (error) {
    return jsonResponse({ success: false, error: String(error) }, 400);
...
registerVersionedRoute("/api/approvals/reject", "POST", httpAction(async (ctx, request) => {
  ...
  try {
    const body = await request.json() as { activityId: string };
    const result = await ctx.runMutation(api.approvals.reject, {
      sessionToken,
      activityId: body.activityId as any,
    });
    return jsonResponse(result, result.success ? 200 : 400);
...
registerVersionedCors("/api/organizations/by-id");
registerVersionedCors("/api/organizations/update");
registerVersionedCors("/api/organizations/add-agent");
registerVersionedCors("/api/organizations/remove-agent");
registerVersionedCors("/api/approvals");
registerVersionedCors("/api/approvals/history");
registerVersionedCors("/api/approvals/stats");
